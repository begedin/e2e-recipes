...
const checkoutSandbox = () => cy
  .server({
    onAnyRequest: ((route, proxy) => {
      const sandboxId = Cypress.env('sandboxId');
      if (!sandboxId) { return; }
      proxy.setRequestHeader('Sandbox', sandboxId);
    }),
  })
...
const checkinSandboxCommand = () => cy.request({
  method: 'DELETE',
  url: apiUrl('sandbox'),
...
const createCommand = (schema: string, attributes: {}) => cy
  .request(createRequest(schema, attributes))
